...
if web is not None:
    # У discord.Client нет add_listener — оборачиваем существующий on_ready,
    # чтобы сервер StaffWork стартовал вместе с ботом (и не сломал on_ready бота).
    _sw_original_on_ready = getattr(client, "on_ready", None)

    async def _sw_on_ready_wrapped():
        await _sw_on_ready()
        if _sw_original_on_ready is not None:
            await _sw_original_on_ready()

    client.on_ready = _sw_on_ready_wrapped
