...
class VerifyView(ui.View):
    ...
    async def verify(self, interaction: discord.Interaction, button):
        ...
        verified = get(guild.roles, name=ROLE_VERIFIED)
        newbie = get(guild.roles, name=ROLE_NEWBIE)

        if verified is None:
            await interaction.response.send_message(
                "⚠️ Роль Verified не найдена. Запусти setup_server.py.",
                ephemeral=True)
            return
...
async def on_member_join(member: discord.Member):
    newbie = get(member.guild.roles, name=ROLE_NEWBIE)
    if newbie:
        try:
...
async def on_ready():
    ...
    first = client.guilds[0] if len(client.guilds) == 1 else None
    if first:
    ...
    print("\n✅ Бот готов. Нажми Ctrl+C для остановки.\n")
...
if __name__ == "__main__":
    try:
        ...
        print("❌ Неверный токен. Проверь DISCORD_TOKEN в .env")
